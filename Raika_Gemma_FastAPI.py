...
async def assess_search_requirement(user_input, initial_response=None, language=None):
    ...
    if language == "ko":
        ...
        3. 만약 웹 검색이 필요하다면, 어떤 검색 키워드(쉼표로 구분된 3-5개)를 사용하는 것이 가장 효과적일까요?
           - **멀티 쿼리 전략**: 한국어 질문이라도 (한국 고유의 컨텐츠가 아니라면) 정보의 양을 늘리기 위해 **영어(English) 키워드**를 반드시 섞어서 생성해주세요. (특히 영화, 기술, 학술 정보 등)
           - 예시: "문맹 대신 편지 써주는 남미 영화" -> "남미 영화 편지 대필, South American film letter writing instead of illiterate"
